...
END {
    ...
    # build an array to cover the cost of movement
    previousMoveCost=0;
    movementCost[0]=0;
    for (i=1;i<rightLocation+1;i++) {
        movementCost[i]=previousMoveCost+i;
        previousMoveCost=movementCost[i];
        # print i,movementCost[i];
    }
    ...
    for (locationIndex=leftLocation;locationIndex<rightLocation+1;locationIndex++) {
        ...
        for (moverIndex=leftLocation;moverIndex<rightLocation+1;moverIndex++) {
            ...
            # number of subs * movementCost[distance] (which is no longer linear)
            fuelCost=location[moverIndex] * movementCost[ abs(locationIndex-moverIndex) ];
