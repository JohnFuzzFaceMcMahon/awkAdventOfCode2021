...
END {
    ...
    for (i=1;i<NR+1;i++) {
        ...
        if ( lineStart[i "|Y"]==lineEnd[i "|Y"] ) {
            ...
            # flag to indicate this line was plotted
            iPlottedThisLine[i]=1;
    ...
    # plot diagonal lines 
    for (i=1;i<NR+1;i++) {
        # did we plot it already ?
        if ( iPlottedThisLine[i]!=1 ) {

            # Y=MX+B
            # print "FROM " lineStart[i "|X"] "," lineStart[i "|Y"] " TO " lineEnd[i "|X"] "," lineEnd[i "|Y"]
            # calculate the slope of the line (either 1 or -1) aka M
            deltaX=lineStart[i "|X"] - lineEnd[i "|X"];
            deltaY=lineStart[i "|Y"] - lineEnd[i "|Y"];
            slope=deltaY/deltaX;
            # print "### slope " slope;

            # Figure out the Y Intercept (aka B)
            yIntercept=lineStart[i "|Y"] - slope*lineStart[i "|X"];

            # calculate the range of X
            xLineStart=min( lineStart[i "|X"], lineEnd[i "|X"] );
            xLineEnd=max( lineStart[i "|X"], lineEnd[i "|X"] );

            # print "### X from " xLineStart " to " xLineEnd;
            for(j=xLineStart;j<xLineEnd+1;j++) {
                currentY=slope*j+yIntercept;
                # print "### X " j;
                # print "### Y " currentY;
                # print "### Plotting " j "," currentY;
                plot[ j "," currentY ]=plot[ j "," currentY ]+1;
                # print "";
                # printPlot();
            }
            # print "";
